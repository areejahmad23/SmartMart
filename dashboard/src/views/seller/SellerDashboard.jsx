...
import { FaUsers, FaRegChartBar, FaComment } from "react-icons/fa";
...
import { 
  ...
  get_seller_weekly_stats,
  get_seller_monthly_stats,
  get_seller_daily_stats
...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (timeRange === 'weekly') {
      dispatch(get_seller_weekly_stats());
    } else if (timeRange === 'monthly') {
      dispatch(get_seller_monthly_stats());
    }
    dispatch(get_seller_daily_stats());
  }, [dispatch, timeRange]);
  ...
  const formatComparisonChartData = () => {
    const stats = timeRange === 'weekly' ? sellerWeeklyStats : sellerMonthlyStats;
    ...
    if (timeRange === 'weekly') {
      ...
      const orderedStats = daysOfWeek.map(day => {
        const dayData = stats.find(item => item.day === day) || {
          orders: 0,
          revenue: 0
        };
        return dayData;
      ...
      const orderedStats = months.map(month => {
        const monthData = stats.find(item => item.month === month) || {
          orders: 0,
          revenue: 0
        };
        return monthData;
  ...
  const getDailyMetrics = () => {
    ...
    return {
      revenue: sellerDailyStats.revenue || 0,
      orders: sellerDailyStats.orders || 0,
      visitors: 1
    };
  };
    ...
    <div className='px-2 md:px-7 py-5 bg-[#f8f9fa]'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-7'>
        ...
        <div className='flex justify-between items-center p-5 bg-gradient-to-r from-[#000022] to-[#0055aa] rounded-lg gap-3 shadow-lg hover:shadow-xl transition-all'>
          ...
          <div className='w-12 h-12 rounded-full bg-white flex justify-center items-center text-xl'>
            <FaComment className='text-[#000033]' />
          </div>
        </div>
      </div>
        ...
        <div className='w-full lg:w-4/12 flex flex-col gap-5'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <TrendCard
              ...
              value={sellerDailyStats ? dailyMetrics.orders : '--'}
              change={sellerDailyStats ? 10 : 0}
              icon={<FaCartShopping className="text-blue-600" />}
            />
            <TrendCard
              title="Conversion Rate"
              value={sellerDailyStats ? `${((dailyMetrics.orders / dailyMetrics.visitors) * 100).toFixed(1)}%` : '--%'}
              change={sellerDailyStats ? 2.5 : 0}
              icon={<FaRegChartBar className="text-purple-600" />}
            />
            <TrendCard
              title="Avg. Order Value"
              value={sellerDailyStats ? `$${(dailyMetrics.revenue / (dailyMetrics.orders || 1)).toFixed(2)}` : '--'}
              change={sellerDailyStats ? 7.8 : 0}
      ...
      <div className='w-full flex flex-col lg:flex-row gap-5 mt-5'>
        ...
        <div className='w-full lg:w-6/12 bg-white p-5 rounded-lg shadow-sm border border-gray-100'>
          <div className='flex justify-between items-center mb-5'>
            ...
            <Link to="/seller/messages" className='text-sm font-medium text-blue-600 hover:text-blue-800'>
              View All
            </Link>
          </div>
